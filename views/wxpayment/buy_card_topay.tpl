...
<div class="ui-container">
	...
	<div class="ui-font17 ui-btn" style="display:none;" id="aliPay">
		<a href="{url action='alipayment/cardBuyWebPay'}?card_id={$card['card_id']}&payment_account_id={$paymentAccountAli['payment_account_id']}&money={$money}&uid={$uid}" class="J-btn-submit btn mj-submit btn-strong btn-larger btn-block">确认支付<span>{$money/100}</span>元</a>
	</div>
</div>
		 ...
		 WeixinJSBridge.invoke(
				 ...
				 function(res){
					 ...
					 if(res.err_msg == "get_brand_wcpay_request:ok") {
						 ...
						 url = wx_url+"/payment/buyCardPaySuccess?card_id="+{$card.card_id};
						 {literal}
						 window.location.href=url;
					 } else {
					 {/literal}
						 url = wx_url+"/payment/buyCardPayFail?card_id="+{$card.card_id}+'&card_type_sell_rule_id='+{$card_type_sell_rule_id};
						 {literal}
						 window.location.href=url;
					 }
				 }
