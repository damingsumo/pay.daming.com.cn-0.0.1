...
<div class="ui-container">
	<ul class="ui-item">
		<li class="ui-boxhor ui-font15 ui-cont">
			...
			<div class="ui-item-right ui-red">￥{$orderSell.price/100}</div>
		</li>
	</ul>
	...
	<div class="ui-font13 ui-note">
		请在<span class="ui-red" id="expire_time">00分00秒</span>内完成付款
	</div>
	
	<div class="ui-font17 ui-btn" style="display:block;" id="wxPay">
		<a onclick="checkActivity();" data="1" id="pay">确认支付<span>{$orderSell.price/100}</span>元</a>
	</div>
	<div class="ui-font17 ui-btn" style="display:none;" id="aliPay">
		<a href="{url action='alipayment/orderDealPay'}?oid={$order['oid']}&payment_account_id={$paymentAccountAli['payment_account_id']}" class="J-btn-submit btn mj-submit btn-strong btn-larger btn-block">确认支付<span>{$orderSell.price/100}</span>元</a>
	</div>
</div>
		 ...
		 WeixinJSBridge.invoke(
				 ...
				 function(res){
					 ...
					 if(res.err_msg == "get_brand_wcpay_request:ok") {
						 ...
						 url = wx_url+"/payment/dealOrderPaysuccess?oid="+{$order.oid}+'&buy_phone='+{$orderSell.buy_phone};
						 {literal}
						 window.location.href=url;
					 } else {
					 {/literal}
						 url = wx_url+"/payment/dealOrderPayfail?oid="+{$order.oid}+'&buy_phone='+{$orderSell.buy_phone}+'&order_sell_id='+{$orderSell.order_sell_id};
						 {literal}
						 window.location.href=url;
					 }
				 }
