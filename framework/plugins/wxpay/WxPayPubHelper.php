...
class Common_util_pub
{
	 ...
	 */
	function postXmlSSLCurl($xml, $url, $second=30, $certPath, $keyPath)
	{
		$ch = curl_init();
		//超时时间
		curl_setopt($ch,CURLOPT_TIMEOUT,$second);
		...
		$certUrl = $enviroment == 'online' ?  CERT_PATH : TEST_CERT_PATH;
		curl_setopt($ch,CURLOPT_SSLCERTTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLCERT, $certUrl.$certPath);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $certUrl.$keyPath);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub 
{
	 ...
	 * 	作用：使用证书post请求xml
	 */
	function postXmlSSL($appid, $mchid, $key, $certPath, $keyPath)
	{
	    $xml = $this->createXml($appid, $mchid, $key);
	    $this->response = $this->postXmlSSLCurl($xml, $this->url, $this->curl_timeout, $certPath, $keyPath);
	    return $this->response;
	}
...
{	
	 ...
	 * 获取prepay_id
	 */
	function getPrepayId($appid, $mchid, $key)
	{
...
class Refund_pub extends Wxpay_client_pub
{
	 ...
	 * 	作用：获取结果，使用证书通信
	 */
	function getResult($appid, $mchid, $key, $certPath, $keyPath)
	{		
		$this->postXmlSSL($appid, $mchid, $key, $certPath, $keyPath);
		$this->result = $this->xmlToArray($this->response);
		return $this->result;
	}
...
class JsApi_pub extends Common_util_pub
{
	 ...
	 * 	作用：设置jsapi的参数
	 */
	public function getParameters($appid, $key)
	{
